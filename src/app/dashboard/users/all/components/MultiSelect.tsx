  ...
  return (
    <div className="space-y-4">
            ...
            <SelectContent>
              <SelectGroup>
                {getAvailableOptions().length === 0 ? (
                  <SelectItem value="user" disabled className="p-2 text-center">
      ...
      {selectedItems.length > 0 && (
        <div className="flex flex-wrap items-start gap-4 -ml-2 px-4">
          <div className="col-span-3 flex flex-wrap gap-2">
            {selectedItems.map((item, index) => (
              <Badge key={`selected-${item}-${index}`} variant="outline">
                {item}
                {!readOnly && (
