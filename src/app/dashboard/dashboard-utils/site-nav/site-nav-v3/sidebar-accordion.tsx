  ...
  return (
    <Accordion type="single" collapsible>
      <AccordionItem value="item-1" className={`max-w-[220px] border-b-0 ${className}`}>
        <AccordionTrigger onClick={handleToggle} className="w-[248px] rounded-[.5rem] py-[.35rem]  hover:no-underline pr-2 transition-colors duration-200">
              ...
              {content?.map((curr: LINKTYPE, index: number) => {
                return (
                  <Link
                    key={curr.id || index}
                    href={`${curr.link}`}
                    className="flex items-center justify-start gap-2 rounded py-[.35rem] pl-[45px] hover:text-primary transition-all duration-200"
                  >
                    ...
                    {curr.badge && (
                      <Badge
                        variant="outline"
                        className="rounded-[.25rem] border border-accent bg-primary py-0 text-[.60rem] text-[#005585] transition-all duration-200"
                      >
                        {curr.badge}
                      </Badge>
                    )}
