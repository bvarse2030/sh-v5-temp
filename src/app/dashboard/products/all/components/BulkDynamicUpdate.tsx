...
const BulkDynamicUpdateNextComponents: React.FC = () => {
  ...
  const { toggleBulkDynamicUpdateModal, isBulkDynamicUpdateModal, bulkData, setBulkData } = useProductsStore();
        ...
        <ScrollArea className="h-[400px] w-full rounded-md border p-4">
          <div className="flex flex-col gap-2">
            {bulkData.map((Products, idx) => (
              <div key={(Products._id as string) || idx} className="flex items-start mb-2 justify-between flex-col">
                <div className="flex flex-col">
                  <span>
                    {idx + 1}. {(Products.name as string) || ''}
                  </span>
                  {/* <span className="text-xs mt-0">{Array.isArray(Products.dataArr) ? products.dataArr.join(', ') : ''}</span> */}
